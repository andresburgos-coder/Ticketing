...
describe('AdminReservationsComponent', () => {
  ...
  beforeEach(async () => {
    const adminServiceSpy = jasmine.createSpyObj('AdminService', ['getReservations']);
    const eventServiceSpy = jasmine.createSpyObj('EventService', ['loadEvents', 'events']);
  ...
  it('should load reservations on init', () => {
    eventService.events.and.returnValue([]);
    adminService.getReservations.and.returnValue(of({
      data: [],
  ...
  it('should change page', () => {
    adminService.getReservations.and.returnValue(of({
      data: [],
      pagination: { page: 2, totalPages: 3, total: 20 }
    } as any));
