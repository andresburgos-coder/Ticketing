      ...
      fc.assert(
        fc.property(
          ...
          (data: any) => {
            ...
            const tickets = data.tickets.map((ticketData: any) =>
              new Ticket(
                ticketData.id,
                ticketData.code,
                ticketData.eventId,
            ...
            for (const ticket of queriedTickets) {
              ...
              expect([TicketType.VIP, TicketType.GENERAL, TicketType.EARLY_BIRD])
      ...
      fc.assert(
        fc.property(
          ...
          (data: any, otherBuyerEmail: string) => {
            ...
            const queriedTickets = tickets.filter((t: Ticket) => t.buyerEmail.equals(buyerEmail));
      ...
      fc.assert(
        fc.property(
          ...
          (data: any) => {
            // Arrange: Create Ticket entities from generated data
            const buyerEmail = Email.create(data.buyerEmail);
            const originalTickets = data.tickets.map((ticketData: any) =>
              new Ticket(
                ticketData.id,
                ticketData.code,
                ticketData.eventId,
            ...
            for (let i = 0; i < queriedTickets.length; i++) {
              ...
              expect(queried.purchaseDate.getTime()).toBe(original.purchaseDate.getTime());
            }
          }
        ),
        PROPERTY_CONFIG
