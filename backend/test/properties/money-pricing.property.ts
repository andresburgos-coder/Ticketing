      ...
      fc.assert(
        fc.property(
          ...
          (baseAmount, currency, quantity) => {
            ...
            const expectedAmount = baseAmount * quantity * PRICING_MULTIPLIERS.VIP;

            // Act
            const totalPrice = basePrice.multiply(quantity).multiply(PRICING_MULTIPLIERS.VIP);
      ...
      fc.assert(
        fc.property(
          ...
          (baseAmount, currency, quantity) => {
            // Arrange
            const basePrice = Money.create(baseAmount, currency);
            const expectedAmount = baseAmount * quantity * PRICING_MULTIPLIERS.GENERAL;

            // Act
            const totalPrice = basePrice.multiply(quantity).multiply(PRICING_MULTIPLIERS.GENERAL);
      ...
      fc.assert(
        fc.property(
          ...
          (baseAmount, currency, quantity) => {
            // Arrange
            const basePrice = Money.create(baseAmount, currency);
            const expectedAmount = baseAmount * quantity * PRICING_MULTIPLIERS.EARLY_BIRD;

            // Act
            const totalPrice = basePrice.multiply(quantity).multiply(PRICING_MULTIPLIERS.EARLY_BIRD);
      ...
      fc.assert(
        fc.property(
          ...
          (baseAmount, currency, quantity) => {
            ...
            // Act
            const vipPrice = basePrice.multiply(quantity).multiply(PRICING_MULTIPLIERS.VIP);
            const generalPrice = basePrice.multiply(quantity).multiply(PRICING_MULTIPLIERS.GENERAL);
            const earlyBirdPrice = basePrice.multiply(quantity).multiply(PRICING_MULTIPLIERS.EARLY_BIRD);
      ...
      fc.assert(
        fc.property(
          ...
          (baseAmount, currency, quantity, ticketTypePricing) => {
            // Arrange
            const basePrice = Money.create(baseAmount, currency);
            const expectedAmount = baseAmount * quantity * ticketTypePricing.multiplier;

            // Act
            const totalPrice = basePrice.multiply(quantity).multiply(ticketTypePricing.multiplier);
      ...
      fc.assert(
        fc.property(
          ...
          (baseAmount, currency, quantity, ticketTypePricing) => {
            ...
            // Act
            const result1 = basePrice.multiply(quantity).multiply(ticketTypePricing.multiplier);
            const result2 = basePrice.multiply(quantity * ticketTypePricing.multiplier);
