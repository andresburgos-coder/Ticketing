...
import {
  IReservationState,
  Reservation,
} from "../../src/domain/states/reservation-state.interface";
      ...
      fc.assert(
        fc.property(
          ...
          ({ initialState, action }) => {
            ...
            switch (action) {
              case "confirm":
                expect(() => terminalState.confirm(mockReservation)).toThrow(
                  InvalidStateTransitionException,
                );
                expect(() => terminalState.confirm(mockReservation)).toThrow(
                  `Cannot confirm reservation in ${initialState} state`,
                );
                break;
              case "cancel":
                expect(() => terminalState.cancel(mockReservation)).toThrow(
                  InvalidStateTransitionException,
                );
                expect(() => terminalState.cancel(mockReservation)).toThrow(
                  `Cannot cancel reservation in ${initialState} state`,
                );
                break;
              case "expire":
                expect(() => terminalState.expire(mockReservation)).toThrow(
                  InvalidStateTransitionException,
                );
                expect(() => terminalState.expire(mockReservation)).toThrow(
                  `Cannot expire reservation in ${initialState} state`,
                );
                break;
            }
          },
        ...
        fc.property(
          ...
          (status, action) => {
            ...
            switch (action) {
              case "confirm":
                if (shouldThrow) {
                  expect(() => state.confirm(mockReservation)).toThrow(
                    InvalidStateTransitionException,
                  );
                } else {
              ...
              case "cancel":
                if (shouldThrow) {
                  expect(() => state.cancel(mockReservation)).toThrow(
                    InvalidStateTransitionException,
                  );
                } else {
              ...
              case "expire":
                if (shouldThrow) {
                  expect(() => state.expire(mockReservation)).toThrow(
                    InvalidStateTransitionException,
                  );
                } else {
