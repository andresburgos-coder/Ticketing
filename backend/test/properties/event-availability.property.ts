      ...
      fc.assert(
        fc.property(
          ...
          (eventData: any, quantity: number) => {
            ...
            const allTypes = [TicketType.VIP, TicketType.GENERAL, TicketType.EARLY_BIRD];
            const nonExistingType = allTypes.find(type => !existingTypes.has(type));

            // Act & Assert
            if (nonExistingType) {
              expect(() => event.reserveTickets(nonExistingType, quantity))
        ...
        fc.property(
          ...
          (eventData: any, quantity: number) => {
            ...
            if (nonExistingType) {
              ...
              expect(() => event.releaseTickets(nonExistingType, quantity))
                .not.toThrow();
      ...
      fc.assert(
        fc.property(
          ...
          (eventData: any) => {
            ...
            const existingTypes = new Set(ticketConfigs.map((c: any) => c.type));
            const allTypes = [TicketType.VIP, TicketType.GENERAL, TicketType.EARLY_BIRD];
            const nonExistingTypes = allTypes.filter(type => !existingTypes.has(type));
