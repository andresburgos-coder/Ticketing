 ...
 */
export const jwtPayloadArbitrary = fc.tuple(userIdArbitrary, emailArbitrary, roleArbitrary).map(
  ([userId, email, role]) => ({
    sub: userId,
    email,
    role,
 ...
 */
export const timestampArbitrary = fc.integer({ min: 1000000000, max: 2000000000 });
 ...
 */
export const jwtTokenArbitrary = jwtPayloadWithTimestampsArbitrary.map((payload) => {
...
export const invalidJwtTokenArbitrary = fc.oneof(
  ...
  fc.tuple(fc.stringMatching(/^[a-zA-Z0-9]{5,}$/), fc.stringMatching(/^[a-zA-Z0-9]{5,}$/)).map(
    ([a, b]) => `${a}.${b}`
  ),
