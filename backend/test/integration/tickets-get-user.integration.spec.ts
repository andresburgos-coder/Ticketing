...
describe("TicketController - GET /tickets/me and /tickets/user (Integration)", () => {
  ...
  beforeAll(async () => {
    ...
    eventRepository =
      moduleFixture.get<TypeOrmEventRepository>(EVENT_REPOSITORY);
    ticketRepository =
      moduleFixture.get<TypeOrmTicketRepository>(TICKET_REPOSITORY);
  ...
  describe("GET /tickets/me", () => {
    ...
    it("should return only tickets for the authenticated user", async () => {
      ...
      expect(response.body).toHaveLength(2);
      expect(
        response.body.every((ticket) => ticket.buyerEmail === testUser.email),
      ).toBe(true);
      ...
      expect(
        anotherResponse.body.every(
          (ticket) => ticket.buyerEmail === anotherUser.email,
        ),
      ).toBe(true);
    });
  ...
  describe("Performance & Multiple Tickets", () => {
    ...
    it("should sort tickets by purchase date (most recent first)", async () => {
      ...
      const dates = response.body.map((t) =>
        new Date(t.purchaseDate).getTime(),
      );
