...
export class ReservationExpirationScheduler {
  ...
  async handleExpiredReservations(): Promise<void> {
    console.log(
      "⏰ [ReservationScheduler] Checking for expired reservations...",
    );

    try {
      // Find all expired reservations (status = PENDING and expiresAt < now)
      const expiredReservations =
        await this.reservationRepository.findExpired();
      ...
      console.log(
        `🔄 [ReservationScheduler] Found ${expiredReservations.length} expired reservation(s)`,
      );

      for (const reservation of expiredReservations) {
        try {
          console.log(
            `📤 [ReservationScheduler] Processing expired reservation: ${reservation.id}`,
          );
          ...
          console.log(
            `📡 [ReservationScheduler] Broadcasting availability update: ${newAvailability} for ${reservation.ticketType}`,
          );
          ...
          console.log(
            `✅ [ReservationScheduler] Released ${reservation.quantity.value} tickets for reservation ${reservation.id}`,
          );
        } catch (error) {
          console.error(
            `❌ [ReservationScheduler] Error processing reservation ${reservation.id}:`,
            error,
          );
        }
      }
      ...
      console.error(
        "❌ [ReservationScheduler] Error checking expired reservations:",
        error,
      );
    }
  }
