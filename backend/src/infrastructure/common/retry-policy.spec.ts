...
describe('RetryPolicy', () => {
  describe('execute', () => {
    it('should succeed on first attempt', async () => {
      // Arrange
      const policy = new RetryPolicy({ maxAttempts: 3, initialDelayMs: 10, backoffMultiplier: 2 });
    ...
    it('should handle non-Error exceptions', async () => {
      // Arrange
      const policy = new RetryPolicy({ maxAttempts: 2, initialDelayMs: 10, backoffMultiplier: 2 });
  ...
  describe('exponential backoff', () => {
    ...
    it('should not delay after final failed attempt', async () => {
      // Arrange
      const policy = new RetryPolicy({ maxAttempts: 2, initialDelayMs: 50, backoffMultiplier: 2 });
      const operation = jest.fn().mockRejectedValue(new Error('Always fails'));
  ...
  describe('error handling', () => {
    ...
    it('should handle non-Error objects in executeWithFallback', async () => {
      // Arrange
      const policy = new RetryPolicy({ maxAttempts: 1, initialDelayMs: 10, backoffMultiplier: 2 });
      const operation = jest.fn().mockRejectedValue('string error');
