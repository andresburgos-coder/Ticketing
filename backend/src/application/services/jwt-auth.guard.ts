...
export class JwtAuthGuard implements CanActivate {
    ...
    async canActivate(context: ExecutionContext): Promise<boolean> {
        ...
        if (token.includes('{{') || token.includes('}}')) {
            ...
            throw new UnauthorizedException('Token contains unresolved variable. Please login first to get a valid token.');
        }

        try {
            this.logger.debug(`Attempting to verify token: ${token.substring(0, 20)}...`);
            this.logger.debug(`Using JWT_SECRET: ${this.JWT_SECRET.substring(0, 10)}...`);
